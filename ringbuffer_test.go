...
func TestNewRingBuffer_A1_ValidCapacity(t *testing.T) {
	t.Parallel()
...
func TestNewRingBuffer_A2_CapacityOne(t *testing.T) {
	t.Parallel()
	rb := carousel.NewRingBuffer[[]byte](1)
	assert.Equal(t, 1, rb.Cap())
}

func TestNewRingBuffer_A3_PanicsOnZeroCapacity(t *testing.T) {
	t.Parallel()
	assert.Panics(t, func() { carousel.NewRingBuffer[int](0) })
}

func TestNewRingBuffer_A4_PanicsOnNegativeCapacity(t *testing.T) {
	t.Parallel()
	assert.Panics(t, func() { carousel.NewRingBuffer[int](-1) })
}
...
func TestPush_B2_ReturnsFalseWhenFull(t *testing.T) {
	t.Parallel()
	rb := carousel.NewRingBuffer[int](2)
	rb.Push(1)
	rb.Push(2)
...
func TestForcePush_C4_SingleCapacityAlwaysEvicts(t *testing.T) {
	t.Parallel()
	rb := carousel.NewRingBuffer[int](1)
	rb.Push(1)
...
func TestLenCap_H2_CapIsConstant(t *testing.T) {
	t.Parallel()
	rb := carousel.NewRingBuffer[int](7)
	for range 7 {
		rb.Push(0)
	}
...
func BenchmarkRingBuffer_Push(b *testing.B) {
	rb := carousel.NewRingBuffer[[]byte](512)
	data := make([]byte, 64)
	for range 256 {
		rb.Push(data)
	}
...
func BenchmarkRingBuffer_ForcePush(b *testing.B) {
...
func BenchmarkRingBuffer_Drain(b *testing.B) {
	rb := carousel.NewRingBuffer[[]byte](256)
	data := make([]byte, 64)
	b.ResetTimer()
	for range b.N {
