...
func defaultTargets(root string) []benchTarget {
	return []benchTarget{
		{
			Path:       filepath.Join(root, "docs", "ringbuffer.md"),
			MarkerName: "benchsync:ringbuffer",
			Rows: []benchRow{
				{Name: "BenchmarkRingBuffer_Push", Label: "Push"},
		...
		{
			Path:       filepath.Join(root, "docs", "ringqueue.md"),
			MarkerName: "benchsync:ringqueue",
			Rows: []benchRow{
